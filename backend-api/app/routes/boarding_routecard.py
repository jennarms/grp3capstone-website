...
def _time_based_seat_info(schedule_id, service_date, vehicle_capacity):
    """
    Compute seats by (departure_date + schedule_id) using BoardingDisembarking's status
    for seats with status 'P' (Pending) or 'B' (Boarded), and filter by time.
    """
    cur = mysql.connection.cursor()
    try:
        # Query to count seats with status 'P' (Pending) or 'B' (Boarded) in BoardingDisembarking
        # Only include Pending passengers with boarding_time in the future or NULL
        cur.execute("""
            ...
            WHERE bd.departure_date = %s
              ...
              AND (
                  (bd.status = 'P' AND (bd.boarding_time IS NULL OR bd.boarding_time > NOW()))  -- Pending with future or no boarding time
                  OR bd.status = 'B'  -- Boarded status
              )
        """, (service_date, schedule_id))
        
        row = cur.fetchone()
        seats_taken = int(row[0]) if row else 0
        total = int(vehicle_capacity or 0)
        available = max(0, total - seats_taken)  # Calculate available seats
        
        return {"total": total, "seats_taken": seats_taken, "available": available}
    finally:
        cur.close()
        ...
        try:
            ...
            payload = {
                ...
                "schedule_info": {
                    ...
                    "seats_taken": seat["seats_taken"],  # Correct value for seats taken
